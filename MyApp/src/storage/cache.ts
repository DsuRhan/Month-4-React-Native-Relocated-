...
const PRODUCT_CACHE_KEY = "product_detail_cache_v1";
...
export const loadCategoryCache = async (category: string) => {
  ...
  try {
    ...
    if (!json?.timestamp || !json?.data) return null;
...
// --------------------------
export const saveProductDetailCache = async (productId: number, data: any) => {
  const payload = { timestamp: Date.now(), data };
  await AsyncStorage.setItem(
    `${PRODUCT_CACHE_KEY}_${productId}`,
    JSON.stringify(payload)
  );
};

export const loadProductDetailCache = async (productId: number) => {
